...
//返回ore电机变量地址，通过指针方式获取原始数据
const motor_measure_t *get_Ore_Motor_Measure_Point(void)
{
    return &ore_motor;
}
...
static void CAN_hook1(CanRxMsg *rx_message)
{
    switch (rx_message->StdId)
    {
			...
			case CAN_CLAMP_ID:
			  {
						//处理电机数据宏函数
						get_motor_measure(&clamp_motor, rx_message);
						break;
				}
		  case CAN_ORE_ID:
				{
						//处理电机数据宏函数
						get_motor_measure(&ore_motor, rx_message);
						break;
				}
     default:
				{
